...
关卡 JSON 还保存 `atmosphere`（LightGroup、tonemap、雾、Bloom、Vignette）；每个 Part 保存 `look`（Unlit 分面 Neg/Mid/Pos 与 lightAxis）。这两项都由 Inspector 写入内存文档，再随保存/导出走同一条真相源。默认雾关闭（start=1000、density=0），tonemap 默认 `none`。
