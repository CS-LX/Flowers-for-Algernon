# Credits

第三方资产与授权记录。模型导入、材质和静物登记另做，不写在这里。

## Grave

Grave by Zsky [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/KmXnfJxFHG)

本地源文件：`assets/model/Grave by Zsky - KmXnfJxFHG.glb`

## Algernon（Mouse）

Toy Mouse by sirkitree [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/49PYgABKfFo)

## Daisy

Daisy by Zsky [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/4r0WBz3RZz)

本地源文件：`assets/model/Daisy by Zsky - 4r0WBz3RZz.glb`

## Flower

Flower by Poly by Google [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/eydI4__jXpi)

本地源文件：`assets/model/Flower by Poly by Google - eydI4__jXpi.glb`

## Camera Stand

Camera stand by Jason Oudshoorn [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/5CHQvf_emuN)

本地源文件：`assets/model/Camera stand by Jason Oudshoorn - 5CHQvf_emuN.glb`

## Monitor

Monitor by Poly by Google [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/2D221SHzAER)

本地源文件：`assets/model/Monitor by Poly by Google - 2D221SHzAER.glb`

## Lab Desk

Lab Desk by Colonel Cthulu [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [Poly Pizza](https://poly.pizza/m/6KxaD6I6I48)

本地源文件：`assets/model/LabDesk.fbx`

## Music

感谢以下音乐创作者为本作品提供优秀的音乐素材。上述音乐的著作权归各自权利人所有，本作品依据相应许可使用。

### Writing the future

Writing the future by Pitx [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [ccMixter](https://ccmixter.org/files/Pitx/13147)

本地源文件：`assets/audio/Writing the future (CH1).mp3`

### Brittle Rille

“Brittle Rille” Kevin MacLeod (incompetech.com)
Licensed under Creative Commons: By Attribution 4.0 License
https://creativecommons.org/licenses/by/4.0/

本地源文件：`assets/audio/Brittle Rille (CH2).mp3`

### Piece of Quiet

Piece of Quiet by geoffpeters [CC-BY](https://creativecommons.org/licenses/by/3.0/) via [ccMixter](https://ccmixter.org/files/geoffpeters/31391)

本地源文件：`assets/audio/Piece of Quiet (CH3).mp3`

### Envisaging

Envisaging by Cloria Sound Labs via [DOVA-SYNDROME](https://dova-s.jp/bgm/detail/6543)

Used under the [DOVA-SYNDROME Music License](https://dova-s.jp/help/articles/license/)

本地源文件：`assets/audio/Envisaging (CH4).mp3`

### Eschatology

Eschatology by SOUNDORBIS via DOVA-SYNDROME

Used under the [DOVA-SYNDROME Music License](https://dova-s.jp/help/articles/license/)

本地源文件：`assets/audio/Eschatology (CH4_2).mp3`

### 添い寝

添い寝 by SOUNDORBIS via DOVA-SYNDROME

Used under the [DOVA-SYNDROME Music License](https://dova-s.jp/help/articles/license/)

本地源文件：`assets/audio/添い寝 (CH5).mp3`

### 夢の中ならば

夢の中ならば by SOUNDORBIS via DOVA-SYNDROME

Used under the [DOVA-SYNDROME Music License](https://dova-s.jp/help/articles/license/)

本地源文件：`assets/audio/夢の中ならば (credits).mp3`

### Chained Story

“Chained Story” SOUNDORBIS (DOVA-SYNDROME)

Used under the [DOVA-SYNDROME Music License](https://dova-s.jp/help/articles/license/)

本地源文件：`assets/audio/千里 - Chained Story.mp3`

