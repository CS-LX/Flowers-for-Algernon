...
关卡 Tab 的 Atmosphere 覆盖 LightGroup Zone（不新建 Zone，AutoExposure 关闭，`tonemapMode = TONEMAP_MODE_NONE`）。Part Tab 的 Look 使用 Unlit `dot(世界法线, lightAxis)` 分面渐变，写入 `Part.look`。灰色地面已从关卡编辑、体素编辑和 Preview 移除。编辑器与 Preview 都不开 `hdrRendering`，让 Inspector 色板和体素走同一套 sRGB。
